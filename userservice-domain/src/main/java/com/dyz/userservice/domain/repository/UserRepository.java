...
public interface UserRepository extends JpaRepository<User, Integer> {
    ...
    @Query(value = "select * from user where if(?1 is NULL,1=1,id=?1)"
            ...
            + " and if(?7 is NULL,enable='Y',enable=?7)"
            + " and if(?8 is NULL,1=1,available=?8)", nativeQuery = true)
    List<User> queryUsers(Integer id, String emailAddress, String phoneNumber, String nickName,
                          Date fromRegisterTime, Date toRegisterTime, String enable, String available);
